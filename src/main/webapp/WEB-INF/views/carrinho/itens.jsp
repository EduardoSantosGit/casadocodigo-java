		    ...
		    <table id="cart-table">
		      ...
		      <c:forEach items="${carrinhoCompras.itens}" var="item">
			      <tr>
			          ...
			          <td class="quantity-input-cell"><input type="number" min="0" readonly="readonly" 
			          id="quantidade" name="quantidade" value="${carrinhoCompras.getQuantidade(item)}"/></td>
			          <td class="numeric-cell">${carrinhoCompras.getTotal(item)}</td>
			          <td class="remove-item">
			          <form action="${s:mvcUrl('CCC#remover').arg(0,item.produto.id).arg(1,item.tipoPreco).build()}" method="post">
			          <input src="${contextPath }resources/imagens/excluir.png" type="image" alt="Excluir" title="Excluir" /></td>
			          </form>
			      </tr>
			   </c:forEach>   
		      </tbody>
